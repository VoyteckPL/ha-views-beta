...
const badgeDefaults = () => ({
  ...
  backgroundColor: '#03101A', backgroundOpacity: .76, backgroundGradient: 'none', backgroundStateEnabled: false, backgroundOnColor: '#03101A', backgroundOffColor: '#03101A', backgroundOnOpacity: .76, backgroundOffOpacity: .76,
...
const gaugeDefaults = () => ({
  ...
  showBackground: true, backgroundColor: '#03101A', backgroundOpacity: .76, backgroundGradient: 'none', backgroundStateEnabled: false, backgroundOnColor: '#03101A', backgroundOffColor: '#03101A', backgroundOnOpacity: .76, backgroundOffOpacity: .76,
...
function markerBackgroundFill(color, opacity, variant = 'none') {
  const alpha = clamp(opacity, 0, 1), tone = value => rgba(color, alpha * value);
  if (variant === 'center') return `radial-gradient(ellipse at 50% 50%,${tone(1)} 0%,${tone(.55)} 34%,${tone(.16)} 62%,${tone(0)} 82%)`;
  if (variant === 'corner') return `radial-gradient(circle at 16% 18%,${tone(1)} 0%,${tone(.62)} 22%,${tone(.18)} 49%,${tone(0)} 74%)`;
  if (variant === 'wall') return `linear-gradient(90deg,${tone(1)} 0%,${tone(.55)} 32%,${tone(.16)} 59%,${tone(0)} 82%)`;
  if (variant === 'ambient') return `radial-gradient(ellipse at 72% 30%,${tone(.78)} 0%,${tone(.22)} 43%,${tone(0)} 70%), linear-gradient(145deg,${tone(.25)},${tone(.7)})`;
...
function normalizedStyle(type, raw = {}) {
  ...
  ['width','height','contentScale','baseContentScale','borderWidth','radius','labelScale','valueScale','labelY','valueY','iconSize','iconX','iconY','iconOpacity','iconOutlineWidth','backgroundOnOpacity','backgroundOffOpacity','borderOnOpacity','borderOffOpacity','borderOnWidth','borderOffWidth','iconOnOpacity','iconOffOpacity','iconOutlineOpacity','iconOutlineOnOpacity','iconOutlineOffOpacity','iconOutlineOnWidth','iconOutlineOffWidth'].forEach(k => base[k] = numberOr(base[k], markerStyleDefaults(type)[k]));
...
function applyMarkerStyle(node, marker) {
  ...
  Object.assign(node.style, {
    ...
    background: s.showBackground ? markerBackgroundFill(backgroundColor, backgroundOpacity, s.backgroundGradient) : 'transparent',
